...
export default function AUsupPage() {
  ...
  // --- TAMBAHAN STATE UNTUK MODAL UPLOAD ---
  const [showModalUpload, setShowModalUpload] = useState(false)
  const [pinUpload, setPinUpload] = useState('')
  const [tempFile, setTempFile] = useState<File | null>(null)

  // --- TAMBAHAN STATE UNTUK MODAL LUNASI BATCH ---
  ...
  // --- TAMBAHAN FUNGSI FORMAT TANGGAL UNTUK WAKTU LIVE ---
  ...
  function exportBelumTerbayar() {
    ...
    const namaFile = `A-PARUK-BELUM-DIBAYAR-${tanggal}.xlsx`

    XLSX.writeFile(workbook, namaFile)
  }

  // --- UPLOAD EXCEL (MODIFIED UNTUK MODAL) ---
  function handleSelectFileExcel(event: React.ChangeEvent<HTMLInputElement>) {
    const file = event.target.files?.[0]
    if (!file) return

    setTempFile(file)
    setShowModalUpload(true) // Tampilkan modal password upload
  ...
  async function prosesUploadExcel() {
    ...
    for (const item of jsonData) {
      ...
      // INSERT
      await supabase.from('live_reports_a_paruk').insert([
        {
    ...
    alert('Upload selesai tanpa duplicate')
    setTempFile(null)
    getData()
    setLoading(false)
  }

  // --- LUNASI (MODIFIED UNTUK MODAL) ---
  function handleKlikLunasi(waktu: string) {
    setTempWaktu(waktu)
    setShowModalLunasi(true) // Tampilkan modal password lunasi
  }
  ...
  async function prosesLunasiBatch() {
    ...
    await supabase.from('live_reports_a_paruk').delete().eq('status', tempWaktu)
  ...
  return (
    <main className="min-h-screen bg-white p-4 md:p-8 relative">
      ...
      {/* ========================================== */}
      {/* POP-UP MODAL UNTUK PIN UPLOAD EXCEL */}
      {/* ========================================== */}
      {showModalUpload && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-xl border border-gray-100">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                className="bg-black hover:bg-gray-800 transition-all text-white px-6 py-3 rounded-xl font-semibold shadow-sm"
              >
                Upload
              </button>
            </div>
      ...
      {/* ========================================== */}
      {/* POP-UP MODAL UNTUK PASSWORD LUNASI */}
      {/* ========================================== */}
      {showModalLunasi && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-xl border border-gray-100">
                  ...
                  data.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="p-5 font-semibold text-red-700">
                        {/* FormatTanggal diterapkan di sini supaya format waktu Supabase rapi */}
