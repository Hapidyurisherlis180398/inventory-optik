...
export default function AturHostPage() {
  ...
  async function fetchHost() {
    const { data } = await supabase.from('daftar_host_live_agung').select('*').order('id', { ascending: true })
  ...
  async function tambahHost() {
    ...
    setLoading(true)
    const { error } = await supabase.from('daftar_host_live_agung').insert([
  ...
  async function hapusHost(id: string) {
    ...
    await supabase.from('daftar_host_live_agung').delete().eq('id', id)
    fetchHost()
  }
